...
export default function AdaptiveLearningPage() {
      ...
      return (
        <PageWrapper className="max-w-4xl mx-auto py-12 text-center space-y-6">
          <div className="p-8 rounded-sm bg-[#0A0A0A] border border-white/10 text-white max-w-md mx-auto space-y-4 font-mono">
            ...
            <h2 className="text-2xl font-extrabold uppercase text-white">Personalized Roadmap Required</h2>
            <p className="text-xs text-zinc-400 font-sans leading-relaxed">
      ...
      return (
        <PageWrapper className="max-w-4xl mx-auto py-12 text-center space-y-6">
          <div className="p-8 rounded-sm bg-[#0A0A0A] border border-white/10 text-white max-w-md mx-auto space-y-4 font-mono">
            ...
            <h2 className="text-2xl font-extrabold uppercase text-white">Study Plan Required</h2>
            <p className="text-xs text-zinc-400 font-sans leading-relaxed">
    ...
    return (
      <PageWrapper className="max-w-4xl mx-auto py-12 text-center space-y-6">
        <div className="p-8 rounded-sm bg-[#0A0A0A] border border-rose-500/40 text-white max-w-md mx-auto space-y-4 font-mono">
          ...
          <h2 className="text-2xl font-extrabold uppercase text-white">Adaptive Intelligence Temporarily Unavailable</h2>
          <p className="text-xs text-zinc-400 font-sans leading-relaxed">
  ...
  return (
    <PageWrapper className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 font-mono">
        <Card className="rounded-sm">
          <CardContent className="p-4 space-y-1">
            ...
            <span className="text-[11px] text-muted-foreground font-sans block">Target Role Skills</span>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-sm">
          <CardContent className="p-4 space-y-1">
            ...
            <span className="text-[11px] text-muted-foreground font-sans block">Positive Score Growth</span>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-sm">
          <CardContent className="p-4 space-y-1">
            <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider block flex items-center gap-1">
              <Minus className="h-3.5 w-3.5 text-muted-foreground" /> Stable / Baseline
            </span>
            <div className="text-3xl font-black text-card-foreground">
              {stableCount + insufficientDataCount}
            </div>
            <span className="text-[11px] text-muted-foreground font-sans block">Consistent Proficiency</span>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-sm">
          <CardContent className="p-4 space-y-1">
            ...
            <span className="text-[11px] text-muted-foreground font-sans block">Plan Execution Rate</span>
          </CardContent>
        </Card>
      </div>
      ...
      {nextBestAction && (
        <Card className="border-2 border-[#FFD400]/60 rounded-sm">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2 font-mono">
                  <Badge variant="default" className="text-[10px] uppercase font-bold bg-surface-secondary text-card-foreground border-border">
                ...
                <h3 className="text-2xl font-extrabold text-card-foreground uppercase">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 rounded-sm">
          ...
          <CardContent className="p-6 space-y-4">
            {trends.length === 0 ? (
              <div className="text-center py-8 text-xs text-muted-foreground font-mono">No skill trends analyzed yet.</div>
            ) : (
              <div className="space-y-4">
                {trends.map((item) => {
                  return (
                    <div
                      key={item.skillName}
                      className="p-4 rounded-sm border border-border bg-surface-secondary space-y-3 font-sans"
                    >
                      <div className="flex justify-between items-start">
                        <div className="space-y-0.5">
                          <h4 className="font-bold text-lg text-card-foreground uppercase">
                            {item.skillName}
                          </h4>
                        ...
                        >
                          ...
                          {item.trend === 'DECLINING' && <TrendingDown className="h-3 w-3 text-rose-500" />}
                          {item.trend === 'STABLE' && <Minus className="h-3 w-3 text-card-foreground" />}
                          {item.trend === 'INSUFFICIENT_DATA' && <HelpCircle className="h-3 w-3 text-muted-foreground" />}
                          {item.trend.replace('_', ' ')}
                        </Badge>
                      </div>

                      <Progress value={item.currentProficiency} className="h-1.5 bg-background" />

                      {/* Score History Trail */}
                      <div className="flex items-center justify-between text-[11px] font-mono text-muted-foreground pt-1">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-[10px] text-muted-foreground font-bold uppercase">Assessment History:</span>
                          {item.scoreHistory && item.scoreHistory.length > 0 ? (
                            item.scoreHistory.map((s, idx) => (
                              <span
                                key={idx}
                                className="px-1.5 py-0.5 rounded bg-background font-bold text-card-foreground text-[10px]"
                              >
                                {s}%
                              </span>
                            ))
                          ) : (
                            <span className="text-muted-foreground italic text-[10px]">Unassessed</span>
                          )}
                        </div>
                        ...
                        {item.changePoints !== 0 && (
                          <span
                            className={`font-extrabold ${
                              item.changePoints > 0 ? 'text-[#FFD400]' : 'text-rose-500 dark:text-rose-400'
                            }`}
                          >
          ...
          <Card className="rounded-sm">
            ...
            <CardContent className="p-4 space-y-3 font-sans">
              {activeRecommendations.length === 0 ? (
                <div className="text-center py-6 text-xs text-muted-foreground font-mono">No active recommendations.</div>
              ) : (
                activeRecommendations.map((rec) => (
                  <div
                    key={rec._id}
                    className="p-3.5 rounded-sm border border-border bg-surface-secondary space-y-2"
                  >
                    <div className="flex justify-between items-start gap-1 font-mono">
                      ...
                      <Badge variant="default" className="text-[9px] uppercase font-bold py-0 bg-surface text-card-foreground border-border">
                        {rec.type.replace('_', ' ')}
                      </Badge>
                    </div>

                    <h4 className="font-bold text-base text-card-foreground leading-snug uppercase">
                      {rec.title}
                    </h4>
                    ...
                    <div className="pt-2 flex justify-between items-center border-t border-border font-mono">
                      <button
                        ...
                        className="text-[10px] font-bold uppercase text-muted-foreground hover:text-card-foreground"
                      >
                        Dismiss
                      </button>
                      ...
                      <Button
                        ...
                        variant="primary"
                        className="font-bold text-[10px] uppercase h-6 px-2.5"
                      >
                        {rec.actionLabel} ↗
                      </Button>
                    </div>
                  ...
                  <ul className="space-y-1 font-mono text-[11px]">
                    {improvingSkills.map((s) => (
                      <li key={s.skillName} className="flex justify-between">
                        <span className="text-card-foreground">{s.skillName}</span>
                        <span className="text-[#FFD400] font-bold">+{s.changePoints}%</span>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <span className="text-muted-foreground italic text-[11px]">No skill gains recorded yet.</span>
                )}
              </div>

              <div className="pt-2 border-t border-border">
                <span className="text-[10px] font-bold uppercase text-rose-500 dark:text-rose-400 flex items-center gap-1 mb-1">
                  <Target className="h-3 w-3 text-rose-500 dark:text-rose-400" /> Primary Skill Gaps
                </span>
                {strugglingSkills.length > 0 ? (
                  <ul className="space-y-1 font-mono text-[11px]">
                    {strugglingSkills.slice(0, 3).map((s) => (
                      <li key={s.skillName} className="flex justify-between">
                        <span className="text-card-foreground">{s.skillName}</span>
                        <span className="text-rose-500 dark:text-rose-400 font-bold">Gap: {s.gap}%</span>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <span className="text-muted-foreground italic text-[11px]">No major skill gaps detected.</span>
                )}
              </div>
            </CardContent>
          </Card>
