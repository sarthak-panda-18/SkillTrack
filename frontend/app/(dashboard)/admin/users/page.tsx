        ...
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          ...
          {isLoadingDetails ? (
            ...
            <div className="space-y-6 py-2 text-xs">
              ...
              {/* Career Outcome & Salary Progression */}
              <div className="space-y-2">
                <div className="font-bold text-slate-900 dark:text-slate-100 flex items-center gap-1.5">
                  <Briefcase className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
                  Active Career Outcome & Salary Details
                </div>
                {!userDetailData.careerOutcome ? (
                  <div className="p-3 text-slate-400 border border-dashed border-slate-200 dark:border-slate-800 rounded-lg">
                    No active career outcome recorded by student.
                  </div>
                ) : (
                  <div className="p-3.5 rounded-xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/50 space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <Badge variant="success" className="text-[10px] uppercase font-black">
                          {userDetailData.careerOutcome.outcomeType?.replace(/_/g, ' ')}
                        </Badge>
                        {userDetailData.careerOutcome.verificationStatus && (
                          <Badge variant="purple" className="text-[10px] uppercase font-black">
                            Verification: {userDetailData.careerOutcome.verificationStatus}
                          </Badge>
                        )}
                      </div>
                      {userDetailData.careerOutcome.employment?.compensationAmount && (
                        <div className="font-extrabold text-sm text-emerald-700 dark:text-emerald-300">
                          Current Salary: ₹{(userDetailData.careerOutcome.employment.compensationAmount / 100000).toFixed(1)} LPA
                        </div>
                      )}
                    </div>

                    {userDetailData.careerOutcome.employment && (
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1 text-xs">
                        <div><span className="text-slate-400 font-bold">Company:</span> <span className="font-bold">{userDetailData.careerOutcome.employment.companyName}</span></div>
                        <div><span className="text-slate-400 font-bold">Job Role:</span> <span className="font-bold">{userDetailData.careerOutcome.employment.jobRole}</span></div>
                        <div><span className="text-slate-400 font-bold">Salary Growth:</span> <span className="font-bold text-emerald-600">+{userDetailData.careerOutcome.employment.salaryGrowthPercentage || 0}%</span></div>
                      </div>
                    )}
                  </div>
                )}
              ...
              {/* Longitudinal Outcome History Timeline */}
              {userDetailData.outcomeHistory && userDetailData.outcomeHistory.length > 0 && (
                <div className="space-y-2">
                  <div className="font-bold text-slate-900 dark:text-slate-100 flex items-center gap-1.5">
                    <Calendar className="h-4 w-4 text-indigo-600 dark:text-indigo-400" />
                    Longitudinal Outcome History ({userDetailData.outcomeHistory.length})
                  </div>
                  <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                    {userDetailData.outcomeHistory.map((h: any) => (
                      <div key={h._id} className="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs">
                        <div>
                          <span className="font-bold text-slate-900 dark:text-slate-100">{h.outcomeType?.replace(/_/g, ' ')}</span>
                          <div className="text-[10px] text-slate-500">
                            Recorded {new Date(h.createdAt).toLocaleDateString()} • Verification: {h.verificationStatus || 'N/A'}
                          </div>
                        </div>
                        <Badge variant={h.status === 'ACTIVE' ? 'success' : 'secondary'} className="text-[9px]">
                          {h.status}
                        </Badge>
                      </div>
                    ))}
